...
# Safe imports for PDF, OCR, and PDF-to-Image conversion
try:
    import pdfplumber
except ImportError:
    pdfplumber = None
...
def parse_omr_file(uploaded_file):
    """
    Extracts question-answer pairs from uploaded OMR PDF or Image files.
    Handles scanned OMR image PDFs using OCR fallback if pdfplumber returns no text.
    """
    responses = {}
    ...
    try:
        ...
        # 2. PDF Files
            ...
            # Fallback to OCR if PDF contains a scanned image OMR
            if not extracted_text.strip() and convert_from_bytes is not None and pytesseract is not None:
                images = convert_from_bytes(file_bytes)
                for img in images:
                    extracted_text += pytesseract.image_to_string(img) + "\n"

        # Pattern A: Standard formats "1: A", "1. A", "1 - A", "1 A"
        matches = re.findall(r'(?:^|\b|\s)(\d{1,2})[\s\.\:\-\|]+([A-Da-d])(?:\b|\s|$)', extracted_text)

        # Pattern B: Grid/Table layouts "1 | 2 | 3" followed by "D | A | A"
        if not matches:
            lines = [line.strip() for line in extracted_text.splitlines() if line.strip()]
            for i in range(len(lines) - 1):
                nums = re.findall(r'\b(\d{1,2})\b', lines[i])
                ans_list = re.findall(r'\b([A-Da-d])\b', lines[i + 1])
                if len(nums) > 1 and len(nums) == len(ans_list):
                    for q_num_str, ans in zip(nums, ans_list):
                        matches.append((q_num_str, ans))
